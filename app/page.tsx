...
import { COMPETENCE_CERTIFICAT_MAP, diffCompetencesCertificats, resolveNomFormation } from '@/utils/competenceCertificatConfig'
import { verifierRetraitCompetence, CertificatAssocie, ConfirmationRetrait } from '@/utils/competenceConfirmation'
import ModalConfirmationCompetence from '@/app/components/ModalConfirmationCompetence'
...
export default function ProfilPage() {
  ...
  // États pour compétences → certificats
  const [snapshotAvantCompetences, setSnapshotAvantCompetences] = useState<Record<string, string[]>>({})
  const [certificatsCompetence, setCertificatsCompetence] = useState<CertificatAssocie[]>([])
  const [confirmationRetrait, setConfirmationRetrait] = useState<ConfirmationRetrait | null>(null)
  const [pendingCompetenceChanges, setPendingCompetenceChanges] = useState<{ ajouts: any[]; retraits: any[]; confirmationsRequises: ConfirmationRetrait[] } | null>(null)
  const [showRedirectFormation, setShowRedirectFormation] = useState(false)
 ...
 useEffect(() => {
    const loadData = async () => {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (debugMode === 'true') {
          ...
          if (debugUser) {
            ...
            // Snapshot compétences pour détection des changements
            setSnapshotAvantCompetences(prendreSnapshotCompetences(loaded))
            if (fullData.benevole_id) {
              chargerCertificatsCompetence(fullData.benevole_id).then(setCertificatsCompetence)
            }
      ...
      if (reservisteData.benevole_id) {
        chargerCertificatsCompetence(reservisteData.benevole_id).then(setCertificatsCompetence)
      }
  ...
  // ─── Compétences → Certificats (helpers) ────────────────────────────────

  /** Prend un snapshot des compétences actuelles (convertit IDs → labels) */
  const prendreSnapshotCompetences = (d: DossierData): Record<string, string[]> => {
    const snapshot: Record<string, string[]> = {}
    for (const config of COMPETENCE_CERTIFICAT_MAP) {
      const ids = d[config.champProfil as keyof DossierData]
      if (Array.isArray(ids)) {
        snapshot[config.champProfil] = idsToLabels(config.champProfil, ids as number[])
      } else {
  ...
  const chargerCertificatsCompetence = async (benevoleId: string): Promise<CertificatAssocie[]> => {
    try {
      const { data, error } = await supabase.rpc('get_certificats_competence', { p_benevole_id: benevoleId })
      if (error) { console.error('Erreur chargement certificats compétence:', error); return [] }
      return (data || []) as CertificatAssocie[]
    } catch (err) { console.error('Erreur chargerCertificatsCompetence:', err); return [] }
  }

  /** Applique les créations/désactivations de certificats après save */
  const appliquerChangementsCompetences = async (
    ajouts: Array<{ champProfil: string; label: string; nomFormation: string; aExpiration: boolean }>,
    retraits: Array<{ champProfil: string; label: string }>
  ) => {
    if (!reserviste) return { certificatsCrees: 0, certificatsDesactives: 0, errors: [] as string[] }
    const errors: string[] = []
    let certificatsCrees = 0
    let certificatsDesactives = 0
    const nomComplet = `${dossier.nom} ${dossier.prenom}`
    ...
    for (const ajout of ajouts) {
      try {
        ...
        if (error) { errors.push(`Erreur "${ajout.nomFormation}": ${error.message}`); continue }
        const { id: formationId, is_new: isNew } = result as any
        if (isNew) {
          try {
            const resp = await fetch('https://n8n.aqbrs.ca/webhook/formation-competence', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ action: 'create', benevole_id: reserviste.benevole_id, nom_complet: nomComplet, nom_formation: ajout.nomFormation, supabase_formation_id: formationId }),
            })
            const webhookData = await resp.json()
            if (webhookData?.success && webhookData?.monday_item_id) {
              await supabase.rpc('set_monday_item_id', { p_formation_id: formationId, p_monday_item_id: parseInt(webhookData.monday_item_id) })
            }
          } catch (e) { console.error('Erreur sync Monday (create):', e) }
        }
        certificatsCrees++
      } catch (err: any) { errors.push(`Erreur "${ajout.nomFormation}": ${err.message}`) }
    }

    // B. Désactiver les certificats retirés
    for (const retrait of retraits) {
      try {
        ...
        if (error) { errors.push(`Erreur désactivation "${retrait.label}": ${error.message}`); continue }
        const { action, monday_item_id } = result as any
        if (monday_item_id) {
          try {
            await fetch('https://n8n.aqbrs.ca/webhook/formation-competence', {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ action: action === 'deleted' ? 'delete' : 'mark', monday_item_id: monday_item_id.toString(), benevole_id: reserviste.benevole_id }),
            })
          } catch (e) { console.error('Erreur sync Monday (désactivation):', e) }
        }
        certificatsDesactives++
      } catch (err: any) { errors.push(`Erreur désactivation "${retrait.label}": ${err.message}`) }
    }
  ...
  const handleSave = async () => {
    ...
    if (aDesactiver.length > 0) {
      ...
      for (const r of aDesactiver) {
        if (!retraitsParChamp[r.champProfil]) retraitsParChamp[r.champProfil] = []
        retraitsParChamp[r.champProfil].push(r.label)
      }
      const confirmations: ConfirmationRetrait[] = []
      for (const [champ, labels] of Object.entries(retraitsParChamp)) {
        const c = verifierRetraitCompetence(champ, labels, certificatsCompetence)
        if (c.requiresConfirmation) confirmations.push(c)
      }
      if (confirmations.length > 0) {
        setPendingCompetenceChanges({ ajouts: aCreer, retraits: aDesactiver, confirmationsRequises: confirmations })
        setConfirmationRetrait(confirmations[0])
        return // La modal va s'afficher, on attend la décision
      }
    }
  ...
  const executerSauvegarde = async (
    ...
    competenceRetraits: Array<{ champProfil: string; label: string }>
  ) => {
    setSaving(true)
    setSaveMessage(null)
    setShowRedirectFormation(false)
    ...
    try {
      ...
      if (competenceAjouts.length > 0 || competenceRetraits.length > 0) {
        ...
        if (result.certificatsCrees > 0) {
          setSaveMessage({ type: 'success', text: `Profil sauvegardé ! ${result.certificatsCrees} certificat(s) à compléter dans votre parcours Formation.` })
          setShowRedirectFormation(true)
        } else {
          setSaveMessage({ type: 'success', text: 'Profil sauvegardé avec succès !' })
        }
        if (result.errors.length > 0) console.error('Erreurs sync certificats:', result.errors)
      } else {
        setSaveMessage({ type: 'success', text: 'Profil sauvegardé avec succès !' })
        setSnapshotAvantCompetences(prendreSnapshotCompetences(dossier))
      }
    ...
    setSaving(false)
    setPendingCompetenceChanges(null)
    setConfirmationRetrait(null)
  }

  // ── Handlers modal confirmation retrait compétence ──

  const handleConfirmRetrait = async () => {
    if (!pendingCompetenceChanges) return
    setConfirmationRetrait(null)
    await executerSauvegarde(pendingCompetenceChanges.ajouts, pendingCompetenceChanges.retraits)
  }

  const handleCancelRetrait = () => {
    if (!pendingCompetenceChanges) return
    // Remettre les compétences à l'état d'avant
    const champsARestaurer = pendingCompetenceChanges.confirmationsRequises.map(c => c.champProfil)
    const updates: Partial<DossierData> = {}
    for (const champ of champsARestaurer) {
      const labels = snapshotAvantCompetences[champ] || []
      updates[champ as keyof DossierData] = labelsToIds(champ, labels) as any
    }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7fa' }}>
      ...
      {/* Modal confirmation retrait compétence */}
      {confirmationRetrait && (
        <ModalConfirmationCompetence
          confirmation={confirmationRetrait}
          onConfirm={handleConfirmRetrait}
          onCancel={handleCancelRetrait}
          loading={saving}
        />
      )}
    </div>
