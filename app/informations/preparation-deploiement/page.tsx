...
export default function FormationsEnLignePage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Charger réserviste + modules en parallèle
      const [{ data: res }, { data: mods }] = await Promise.all([
        supabase.from('reservistes').select('benevole_id, prenom, nom, groupe').eq('user_id', authUser.id).single(),
        supabase.from('lms_modules').select('id, titre, description, bucket_path, ordre, certificat').order('ordre'),
      ])

      if (res) setReserviste(res)
      if (mods) setModules(mods)

      // Charger les progressions (besoin du benevole_id)
      if (res) {
