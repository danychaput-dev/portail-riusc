...
//
// Historique des trajets du bénévole connecté + cumul heures secondaires
// et badge éligibilité crédit d'impôt QC.
...
export default function MesTrajetsPage() {
  ...
  const parContexte = new Map<string, Trajet[]>()
  for (const t of trajets) {
    const key = t.deployment_id || t.camp_session_id || 'autre'
    const list = parContexte.get(key) || []
    list.push(t)
    parContexte.set(key, list)
  }
  ...
  if (loading) {
    return (
      <>
        <ImpersonateBanner />
        <PortailHeader subtitle="Mes trajets" />
        <div style={{ padding: 60, textAlign: 'center', color: MUTED }}>Chargement…</div>
      </>
    )
  }
    ...
    <>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '20px 16px 60px' }}>
        ...
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: C }}>
          🚗 Mes trajets de bénévolat
        </h1>
        <p style={{ marginTop: 6, fontSize: 13, color: MUTED, marginBottom: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12, marginBottom: 24 }}>
          ...
          <StatCard label="Complété (en attente d'approbation)" value={formatDuree(Math.max(0, totalMinutesComplete - totalMinutesApprouve))} color={AMBER} bg="#fef3c7" />
          <StatCard label="Total" value={formatDuree(totalMinutesComplete)} color={C} bg="#eff6ff" />
          <StatCard label="Nombre de trajets" value={String(trajets.length)} color="#6366f1" bg="#eef2ff" />
        </div>
              ...
              <tbody>
                {trajets.map(t => (
                  <tr key={t.id} style={{ borderTop: `1px solid ${BORDER}`, opacity: t.statut === 'annule' ? 0.5 : 1 }}>
                    ...
                    <td style={{ ...tdStyle, fontSize: 12, color: MUTED, maxWidth: 300 }}>
                      {t.notes || '—'}
                    </td>
                  </tr>
                ))}
...
function StatCard({ label, value, color, bg }: { label: string; value: string; color: string; bg: string }) {
  return (
    <div style={{ padding: 14, borderRadius: 10, backgroundColor: bg }}>
      <div style={{ fontSize: 11, color: MUTED, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>
